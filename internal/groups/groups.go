...
const (
	...
	// R8：OpenCode 策略组（type=select，服务 opencode.ai 模型调用端点流量；
	// proxies = [手动选择, 自动选择, 地区组名..., 其他节点?(若有), 全部节点名...,
	// 直连组, 拒绝组]——手动组全量 + 首位多一个「手动选择」组；排内置组最后段
	//（其他节点之后、漏网之鱼之前），全局组序见 Build 注释）。
...
//  5. "OpenCode" type=select（R8）：proxies = [手动选择, 自动选择, <地区组名...>,
//     其他节点组(若存在), <全部去重节点名...>, 直连组, 拒绝组]——手动组全量 +
//     首位多一个「手动选择」组，服务 opencode.ai 端点流量；排内置组最后段
//     （其他节点之后、漏网之鱼之前）。
//  6. "漏网之鱼" type=select（R7）：proxies = [手动选择, <地区组名...按出现序>,
//     其他节点组(若存在), <全部去重节点名...>, 直连组, 拒绝组]；首位引用
//     「手动选择」组而非自动选择；任何其他组（含 OpenCode）的 proxies 均不含它。
//  7. "直连"/"拒绝"：type=select，proxies 分别为 [DIRECT]/[REJECT]
//     （R1；DIRECT/REJECT 是 Clash 内置出站，组非空故合法）；声明在漏网之鱼
//     之后、全局最后，拒绝为最后一组。
//  8. 撞名避让（R7 修复轮）：节点名恰为固定组名/动态地区组名（<地区>节点）或内置
...
// 漏网之鱼组与其同构，仅首元素为「手动选择」组（R7 用户定义）；OpenCode 组为
// 手动组全量 + 首位「手动选择」（R8 用户定义）。
//
// 全局组序（R8）：[手动选择, 自动选择, <地区组名...按出现序>, <其他节点?若有>,
// OpenCode, 漏网之鱼, 直连, 拒绝]。
//
// 节点名重复时去重（同一节点只进一个组）。空节点列表仍输出手动选择/自动选择/
// OpenCode/漏网之鱼/直连/拒绝（用户明确要求无节点时也生成直连/拒绝组），不报错。
func Build(nodes []map[string]any) ([]map[string]any, error) {
	...
	// 节点名恰为固定组名（手动选择/自动选择/其他节点/直连/拒绝/漏网之鱼/OpenCode）、
	// 动态地区组名（<地区>节点）或内置出站保留名 DIRECT/REJECT 时，输出配置被
	// mihomo 拒绝
	...
	fixedNames := map[string]bool{
		...
		GroupDirect: true, GroupReject: true, GroupLeak: true, GroupOpenCode: true,
	...
	groups := make([]map[string]any, 0, 6+len(regions)) // 手动+自动+OpenCode+漏网之鱼+直连+拒绝+地区组
	...
	// 4. 手动组 proxies 补全：全部节点名 + 直连组 + 拒绝组
	for _, name := range allNames {
		manualProxies = append(manualProxies, name)
	}
	manualProxies = append(manualProxies, GroupDirect, GroupReject)

	// 5. OpenCode（R8）：proxies = [手动选择, ...手动选择组 proxies 全量]——
	// 首位引用「手动选择」组，其后与手动组一致（自动选择→地区组名→其他节点→
	// 全部节点名→直连组→拒绝组），天然不含漏网之鱼。空节点列表仍生成
	// （[手动选择, 自动选择, 直连, 拒绝]）。
	opencodeProxies := make([]any, 0, len(manualProxies)+1)
	opencodeProxies = append(opencodeProxies, GroupManual)
	opencodeProxies = append(opencodeProxies, manualProxies...)
	opencode := map[string]any{
		"name":    GroupOpenCode,
		"type":    "select",
		"proxies": opencodeProxies,
	}

	// 6. 漏网之鱼（R7）：proxies = [手动选择, ...手动选择组 proxies[1:]]——
	...
	// 7. 直连/拒绝（R1：声明在漏网之鱼之后、全局最后，拒绝为最后一组；
	// DIRECT/REJECT 为 Clash 内置出站，组非空故合法，mihomo 校验通过）
	...
	// 8. 手动选择（proxies 依赖上面组名，最后构建；prepend 到组列表最前）
